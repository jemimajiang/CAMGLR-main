...
class MiniBatchLoader(object):
    ...
    # jm added to load real image
    def load_real_test_data(self,indices):
        return self.load_data(self.testing_path_infos,indices)
    # test ok
    def load_data(self, path_infos, indices, augment=False):
        ...
        if augment: # 函数load_training_data()，mini_batch_size>1
            ...
            for i, index in enumerate(indices):
                ...
                if np.random.rand() > 0.5: # 随机翻转
                    img = np.fliplr(img)

                if np.random.rand() > 0.5: # 随机旋转
                ...
                rand_range_h = h-self.crop_size # 从随机位置裁剪固定尺寸
            ...
            xs = np.zeros((mini_batch_size, in_channels, 512, 512)).astype(np.float32)
            xs[0, 0, :, :] = (img/255).astype(np.float32) # 仅修改图片的量纲
